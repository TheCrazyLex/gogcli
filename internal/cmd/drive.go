...
type DriveCopyCmd struct {
	...
	ConvertTo string `name:"convert-to" help:"Convert copied file to: sheet"`
}
...
type DriveUploadCmd struct {
	...
	Parent    string `name:"parent" help:"Destination folder ID"`
	ConvertTo string `name:"convert-to" help:"Convert uploaded file to: sheet"`
}
...
func driveConvertToMimeType(convertTo string) (string, error) {
	...
	default:
		return "", usagef("invalid --convert-to %q (expected: sheet)", convertTo)
	}
}
